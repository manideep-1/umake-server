...
router.post('/', async (req, res, next) => {
    try {
        ...
        var frontString = req.body.front
        var backString = req.body.back
        ...
        else if(draftId==null)
        {
            if (frontArray.length >= 1) {
                for(var i=0;i<frontArray.length;i++)
                {
                    var customizeAttributesUpdate = await new Customize_Attributes({
                        ...
                        deltaPosition: frontArray[i].deltaPosition,category:"user"
                    })
            ...
            if (backArray.length >= 1) {
                for(var i=0;i<backArray.length;i++)
                {
                    var customizeAttributesUpdate = await new Customize_Attributes({
                        ...
                        deltaPosition: backArray[i].deltaPosition,category:"user"
                    })
                    await customizeAttributesUpdate.save((err, cart) => {
                        if (err)
                            console.log(err)
            ...
            if (frontImgArray.length >= 1) {
                for(var i=0;i<frontImgArray.length;i++)
                {
                    var customizeImgAttributesUpdate = await new CustomizeImg_Attributes({
                        ...
                        width:frontImgArray[i].width,stickerNumber:frontImgArray[i].stickerNumber,
                        deltaPosition: frontImgArray[i].deltaPosition,category:"user"
                    })
            ...
            if (backImgArray.length >= 1) {
                for(var i=0;i<backImgArray.length;i++)
                {
                    var customizeImgAttributesUpdate = await new CustomizeImg_Attributes({
                        ...
                        width:backImgArray[i].width,stickerNumber:backImgArray[i].stickerNumber,
                        deltaPosition: backImgArray[i].deltaPosition,category:"user"
                    })
                    await customizeImgAttributesUpdate.save((err, cart) => {
                        if (err)
                            console.log(err)
            ...
            if (front_ids.length != 0 || back_ids.length != 0 || frontImg_ids.length != 0 || backImg_ids.length != 0) {
                var customizedObjectUpdate = await new Customized_Object({color:color, front: front_ids, back: back_ids, frontImg: frontImg_ids,
                                                                        backImg: backImg_ids, category:"user",frontImgString:frontString,
                                                                        backImgString:backString})
                ...
                var userFind = await User_Drafts.findOne({ "user_id": user_id })
                console.log(userFind)
                if (userFind == null) {
                    var userCreate = await new User_Drafts({ "user_id": user_id, draftIds: [customizedObjectUpdate._id] })
                    await userCreate.save((err, cart) => {
                        if (err)
                            console.log(err)
                        else
                    ...
                    await User_Drafts.findOneAndUpdate({ "user_id": user_id },
                        { $push: { draftIds: customizedObjectUpdate._id } })
                }
            }
        }
        else
        ...
        res.json({response:response,draftId:draftId,images:images})
    }
    catch (err) {
        res.send('Error' + err);
    }
