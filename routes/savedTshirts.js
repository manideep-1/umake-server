...
router.post('/', async (req, res, next) => {
  try {
    ...
    else if(user_drafts!=null)
    {
        async.map(user_drafts.draftIds,function(draftId,done){
            Customized_Object.findById(draftId,function(err,customized_object){
                if(err) return next(err);
                async.series([
                    function(callback) {
                  ...
                  function(err, result) {
                    console.log(result);
                    done(null,{arrayF:result[0],imageColor:customized_object.color,arrayB:result[1],arrayFImg:result[2],arrayBImg:result[3],draftId:draftId,
